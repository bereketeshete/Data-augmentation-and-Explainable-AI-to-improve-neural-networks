...
opt.check_valid_freq = 1; % 1000 How often you will check validation error
...
cnn_output=b';
valid_audio=cnn_output(851:900,:);
...
method='none';% none,noise,resize,
if strcmp(method,'noise')
      for i=1:size(train_audio,1)
          m=train_audio(i);m=m{1};
          m={imnoise(m,'gaussian')};
          data_augment{i,1}=m;
      end
      train_audio=cat(1,cnn_output,cnn_output);% add augmented to the training data
      train_label=repmat(train_label,[2 1]);
elseif strcmp(method,'repeat')
    rep=4;% number of times to repeat training data
